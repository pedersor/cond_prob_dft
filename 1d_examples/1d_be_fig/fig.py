...
if __name__ == '__main__':
  ...
  np.testing.assert_equal(num_dn_electrons, 2)
  ...
  raw_pair_density = utils.to_shape(raw_pair_density, (num_grids, num_grids))
  ...
  for i, ref_point in enumerate(ref_points):
    ...
    if blue_el_results:
      ax1.plot(
          grids,
          cp_ks_blue.density,
          label=r'$\tilde n_y^{\mathrm{BEA, LDA}}(y^{\prime})$',
          color='tab:blue',
      )
    ax1.plot(
        grids,
    ...
    if blue_el_results:
      ax2.plot(
          ...
          label=r'$\tilde v^{\mathrm{BEA, LDA}}_{{\rm S}, y}(y^{\prime})$',
          color='tab:blue',
      )
    ax2.plot(
        grids,
